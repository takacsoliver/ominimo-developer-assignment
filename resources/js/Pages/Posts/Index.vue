<template>
    <Head :title="t('posts.page_title') + ' - Ominimo Blog'" />
        ...
        <div class="min-h-screen">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div class="text-left mb-12">
                    <h1 class="text-4xl font-bold text-gray-900 mb-4">{{ t('posts.page_title') }}</h1>
                </div>
                    ...
                    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <Link v-for="post in posts" :key="post.id" :href="getPostUrl(post.id)" class="group bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 block cursor-pointer">
                            <div class="p-6">
                                ...
                                <h2 class="text-xl font-bold text-gray-900 mb-3 group-hover:text-[#3A30D3] transition-colors duration-200 line-clamp-2">
                                    {{ post.title }}
                                </h2>
                ...
                <div v-if="posts && posts.length > 0 && pagination.last_page > 1" class="mt-12">
                    <div class="flex justify-between items-center mb-4">
                        <p class="text-sm text-gray-700">
                            {{ t('pagination.showing') }} {{ pagination.from }} {{ t('pagination.to') }} {{ pagination.to }} {{ t('pagination.of') }} {{ pagination.total }} {{ t('pagination.results') }}
                        </p>
                        <p class="text-sm text-gray-700">
                    ...
                    <nav class="flex justify-center">
                        ...
                        <template v-for="link in pagination.links" :key="link.label">
                            ...
                            <span
                                v-else
                                :class="[
                                    'px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 opacity-50 cursor-not-allowed',
                                    link.active
                                        ? 'bg-[#3A30D3] text-white'
...
import type { PostsIndexPageProps, Post, Pagination } from '@/types';
